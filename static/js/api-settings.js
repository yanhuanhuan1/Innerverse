...
function applyApimartDefaults(item){
    if(!item) return;
    const protocol = String(item.protocol || '').trim().toLowerCase();
    const baseUrl = String(item.base_url || '').trim().toLowerCase();
    if(protocol !== 'apimart' && !baseUrl.includes('apimart.ai')) return;
    item.id = item.id || 'apimart';
    item.name = item.name || 'APIMart';
    item.base_url = item.base_url || APIMART_DEFAULT_BASE_URL;
    item.protocol = 'apimart';
    item.image_request_mode = item.image_request_mode || 'openai';
    item.primary = true;
    item.image_models = unique([...(item.image_models || []), ...APIMART_DEFAULT_IMAGE_MODELS]);
    item.chat_models = unique([...(item.chat_models || []), ...APIMART_DEFAULT_CHAT_MODELS]);
    item.video_models = unique([...(item.video_models || []), ...APIMART_DEFAULT_VIDEO_MODELS]);
}
...
const RECOMMENDED_APIS = [
    {
        ...
        advantages:['统一接入图像、视频和 LLM 模型', 'OpenAI 兼容请求结构', '适合当前无限画布的一站式生成流程']
    }
];
...
function isApimartOnlyProvider(item){
    const protocol = String(item?.protocol || '').trim().toLowerCase();
    const baseUrl = String(item?.base_url || '').trim().toLowerCase();
    const id = String(item?.id || '').trim().toLowerCase();
    return id === 'apimart' || protocol === 'apimart' || baseUrl.includes('apimart.ai');
}
function visibleProviders(){
    return (providers || []).filter(item => isApimartOnlyProvider(item) && !isProviderTemporarilyHidden(item));
}
...
function addProvider(){
    selectRecommendedApi(RECOMMENDED_APIS.findIndex(api => api.id === 'apimart'));
}
async function addCliProvider(kind){
    setStatus(langIsEn() ? 'Only APIMart is supported in this workspace.' : '当前只保留 APIMart 接口。');
}
function deleteProvider(){
    setStatus(langIsEn() ? 'APIMart is the only API provider and cannot be removed.' : 'APIMart 是唯一接口，不能删除。');
}
...
async function loadProviders(){
    ...
    try {
        ...
        providers = (data.providers || []).filter(isApimartOnlyProvider).map(item => {
            applyApimartDefaults(item);
            return item;
        });
        if(!providers.length){
            providers = RECOMMENDED_APIS.filter(api => api.id === 'apimart').map(api => ({...api, enabled:true, primary:true, has_key:false, key_preview:''}));
            providers.forEach(applyApimartDefaults);
        }
...
async function saveProviders(){
    syncEditor();
    providers = providers.filter(isApimartOnlyProvider);
    if(!providers.length){
        providers = RECOMMENDED_APIS.filter(api => api.id === 'apimart').map(api => ({...api, enabled:true, primary:true, has_key:false, key_preview:''}));
    }
