...
const CANVAS_DARK_DEFAULT_MIGRATED_KEY = 'canvas_dark_default_migrated_v1';
...
function apimartProvider(){
    return (apiProviders || []).find(p => String(p?.id || '').toLowerCase() === 'apimart' || String(p?.protocol || '').toLowerCase() === 'apimart' || String(p?.base_url || '').toLowerCase().includes('apimart.ai')) || null;
}
function composerApiProvider(){
    return apimartProvider() || imageApiProviders()[0] || null;
}
function composerApiImageModels(){
...
function composerModelQuality(model){
    const text = String(model || '').toLowerCase();
    if(text.includes('lite')) return '1K';
    if(text.includes('wan2.7') || text.includes('seedream') || text.includes('gpt-image-2') || text.includes('gemini-3')) return '4K';
    if(text.includes('qwen') || text.includes('z-image')) return '2K';
    return '4K';
}
function composerModelIcon(model){
    const text = String(model || '').toLowerCase();
    if(text.includes('gpt')) return 'sparkles';
    if(text.includes('gemini') || text.includes('banana')) return 'banana';
    if(text.includes('seedream')) return 'bar-chart-3';
    if(text.includes('wan') || text.includes('qwen') || text.includes('z-image')) return 'image';
    return 'sparkles';
}
...
function isFetchNetworkError(err){
    return /Failed to fetch|NetworkError|Load failed|ERR_CONNECTION_REFUSED|ERR_CONNECTION_RESET|Could not connect/i.test(String(err?.message || err || ''));
}
function localServiceUnavailableMessage(){
    return langIsEn()
        ? 'Cannot connect to the local service. Make sure the server is running at http://127.0.0.1:3000/.'
        : '无法连接本地服务。请确认服务已启动：http://127.0.0.1:3000/。';
}
async function fetchJsonChecked(url, init={}, fallback='请求失败'){
    let response;
    try {
        response = await fetch(url, init);
    } catch(err) {
        if(isFetchNetworkError(err)) throw new Error(localServiceUnavailableMessage());
        throw err;
    }
    if(!response.ok) throw new Error(await responseErrorMessage(response, fallback));
    return response.json();
}
async function uploadAiReferenceForm(form, fallback=null){
    return fetchJsonChecked('/api/ai/upload', {method:'POST', body:form}, fallback || (langIsEn() ? 'Upload failed' : '上传失败'));
}
...
function canvasGeneratorClosedLayout(node){
    return {w:520, h:560};
}
function canvasGeneratorOpenLayout(node){
    return canvasGeneratorClosedLayout(node);
}
function normalizeApiNodeLayout(node){
    if(!node) return;
    if(node.type === 'generator' && Number(node.w || 0) === 418) node.w = 380;
    if(node.type === 'comfy'){
        if((node.mode || 'text') === 'text'){
            if(!node.composerModel) node.composerModel = 'banana-2';
            if(!node.composerThinkTime) node.composerThinkTime = 'high';
            if(!node.composerQuality) canvasComposerApplyQuality(node, '2048', node.composerAspect || 'auto');
            if(!node.cameraPreset) node.cameraPreset = 'sony-venice';
        }
        if(!isCanvasGeneratorComposerOpen(node)){
            const size = canvasGeneratorClosedLayout(node);
            node.w = size.w;
            node.h = size.h;
            return;
        }
        const size = canvasGeneratorOpenLayout(node);
        node.w = size.w;
        node.h = size.h;
    }
    if(node.type === 'video'){
...
function resetCanvasPageScroll(){
    if(shell){
        shell.scrollTop = 0;
        shell.scrollLeft = 0;
    }
...
function refreshOutputTimer(){
    const hasPending = nodes.some(n => (n._pending || []).length);
...
function serializableCanvasNodes(list=nodes){
    return (list || []).filter(node => node?.type !== 'output').map(serializableCanvasNode);
}
...
document.addEventListener('mousedown', e => {
    closeCanvasComposerPopoverOnOutsidePointer(e);
}, true);
...
function pickCanvasUploadAtPoint(point, accept='image/*,video/*,audio/*', multiple=true){
    if(!ensureCanvas()) return;
    const input = document.createElement('input');
    input.type = 'file';
    input.accept = accept;
    ...
    input.onchange = () => {
        ...
        if(files.length) uploadImages(files, point || defaultPoint(0, 0));
    };
    input.click();
}
function pickAndConnectUpload(nodeId, event=null, accept='image/*,video/*,audio/*'){
    event?.preventDefault?.();
    event?.stopPropagation?.();
    const target = nodes.find(n => n.id === nodeId);
    if(!target || !ensureCanvas()) return;
    const input = document.createElement('input');
    input.type = 'file';
    input.accept = accept;
    input.multiple = true;
    input.onchange = async () => {
        ...
        const created = await uploadImages(files, {x:Number(target.x || 0) - 320, y:Number(target.y || 0)});
        (created || []).forEach(item => connectCanvasNodes(item.id, target.id));
        selected.clear();
...
async function runMsGenNode(nodeId, opts={}){
    ...
    const prompt = [node.prompt, canvasComposerPromptExtras(node), ...sources.map(s => s.prompt)].filter(Boolean).join('\n\n');
    ...
    outputTarget._pending = [...(outputTarget._pending || []), ...pendingIds.map(id => makePendingForRun(id, run, node, {refs, requestSize, cascadeTargetId}))];
    if(!opts.cascade){
        node.running = true;
    ...
    try {
        ...
        const metas = collectRunMetas(outputTarget, pendingIds);
        const outputUrls = results.map(data => data.url).filter(Boolean);
        run.request = results[0] ? requestMetaFromResult(results[0]) : {};
        outputTarget._pending = (outputTarget._pending || []).filter(p => !pendingIds.includes(p.id));
        if(out) appendOutputImages(out, outputUrls, refs[0], metas);
...
function addComfyNode(point, overrides={}){
    const p = point || defaultPoint(160, 0);
    return addNode({
        id:uid('comfy'),
        type:'comfy',
        ...
        width:2048,
        height:2048,
        composerModel:'banana-2',
        composerQuality:'2048',
        composerAspect:'auto',
        composerThinkTime:'high',
        cameraPreset:'sony-venice',
        enhanceStrength:0.5,
...
function selectedCanvasImageNode(){
    return [...selected]
        .map(id => nodes.find(n => n.id === id))
        .find(n => n?.type === 'image' && n.url && mediaKindForNode(n) === 'image' && !isMissingAssetUrl(n.url)) || null;
}
function canvasCreativePointNearSource(source, dy=0){
    if(!source) return defaultPoint(160, dy);
    return {
        x:Number(source.x || 0) + Number(source.w || defaultNodeSize(source.type).w) + 80,
        y:Number(source.y || 0) + dy
    };
}
function connectCanvasNodes(fromId, toId){
    if(!fromId || !toId || connections.some(c => c.from === fromId && c.to === toId)) return;
...
function canvasReferenceCreateOptions(){
    return [
        {type:'refText', label:'文本生成', desc:'脚本、广告词、品牌文案', icon:'align-left'},
        {type:'refImage', label:'图片生成', desc:'引用当前节点生成图片', icon:'image'},
        {type:'refVideo', label:'视频生成', desc:'引用当前节点生成视频', icon:'play-square'},
        {type:'refImageEdit', label:'图片编辑器', desc:'基于当前图片继续编辑', icon:'image-plus'}
    ];
}
function addCanvasCreativeNode(mode){
    const toolMode = mode || 'text';
    const source = toolMode === 'text' ? null : selectedCanvasImageNode();
    pushUndo();
    const labelMap = {text:'文生图', enhance:'细节增强', edit:'图生图', angle:'角度控制'};
    const point = source ? canvasCreativePointNearSource(source, toolMode === 'enhance' ? 0 : toolMode === 'edit' ? 84 : 168) : (menuPoint || defaultPoint(120, 80));
    const node = addComfyNode(point, {
        mode:toolMode,
        creativeType:toolMode === 'text' ? 'text-to-image' : toolMode,
        title:labelMap[toolMode] || '创作工具',
        sourceNodeId:source?.id || '',
        prompt:toolMode === 'angle' ? 'change the view angle, keep the subject identity and visual style' : '',
        composerOpen:false,
        w:980,
        h:toolMode === 'text' ? 340 : 620
    });
    if(source) connectCanvasNodes(source.id, node.id);
    selected.clear();
...
function activateCanvasCreativeTool(mode){
    document.querySelectorAll('[data-creative-tool]').forEach(btn => btn.classList.toggle('active', btn.dataset.creativeTool === mode));
    addCanvasCreativeNode(mode);
    setTimeout(() => document.querySelectorAll('[data-creative-tool]').forEach(btn => btn.classList.remove('active')), 500);
}
function syncCreativeToolbarState(){
    const hasImage = Boolean(selectedCanvasImageNode());
    document.querySelectorAll('[data-creative-tool]').forEach(btn => {
        const needsImage = btn.dataset.creativeTool !== 'text';
        btn.disabled = needsImage && !hasImage;
        btn.title = needsImage && !hasImage
            ? (langIsEn() ? 'Select an image first' : `${btn.textContent.trim()}：请先选择一张图片`)
            : btn.textContent.trim();
    });
}
function anglePresetPrompt(preset=''){
    const map = {
        front:'front view, keep the same subject identity and visual style',
        side:'side view, keep the same subject identity and visual style',
        top:'top-down view, keep the same subject identity and visual style',
        back:'back view, keep the same subject identity and visual style'
    };
    return map[preset] || 'change the view angle, keep the same subject identity and visual style';
}
function insertCreativeResultImageNodes(sourceNode, outputs=[]){
    const urls = (outputs || []).map(outputUrlValue).filter(Boolean);
    if(!urls.length) return [];
    const sourceW = Number(sourceNode.w || defaultNodeSize(sourceNode.type).w || 360);
    const created = urls.map((url, index) => ({
        id:uid('img'),
        type:'image',
        x:Number(sourceNode.x || 0) + sourceW + 80 + index * 34,
        y:Number(sourceNode.y || 0) + index * 42,
        w:260,
        h:220,
        url,
        name:`${sourceNode.title || '生成结果'} ${index + 1}`,
        sourceNodeId:sourceNode.id,
        status:'success',
        createdAt:Date.now()
    }));
    nodes.push(...created);
    created.forEach(node => connectCanvasNodes(sourceNode.id, node.id));
    selected.clear();
    created.forEach(node => selected.add(node.id));
    return created;
}
function consumeInitialCanvasIntent(){
    let params;
    try { params = new URLSearchParams(window.location.search); } catch(e) { return; }
    const prompt = (params.get('initial_prompt') || '').trim();
    const mode = (params.get('initial_mode') || '').trim() || 'canvas';
    if(!prompt && mode === 'canvas') return;
    params.delete('initial_prompt');
    params.delete('initial_mode');
    const cleanUrl = `${window.location.pathname}?${params.toString()}`;
    window.history.replaceState(null, '', cleanUrl);
    if(mode === 'edit' || mode === 'enhance' || mode === 'angle'){
        const imageNode = addImageNode(defaultPoint(-140, 0));
        if(imageNode){
            imageNode.name = langIsEn() ? 'Reference image' : '参考图';
            selected.clear();
            selected.add(imageNode.id);
        }
        const promptNode = prompt ? addPromptNode(defaultPoint(160, 0)) : null;
        if(promptNode) promptNode.text = prompt;
        setStatus(langIsEn() ? 'Upload a reference image, then run the selected tool.' : '请先上传参考图，再执行选中的创作工具');
        render();
        scheduleSave();
        return;
    }
    const creativeNode = addCanvasCreativeNode('text');
    if(creativeNode){
        creativeNode.prompt = prompt;
        render();
        scheduleSave();
    }
}
...
function openLinkCreateMenu(originId, originKind, clientX, clientY){
    ...
    const options = canvasReferenceCreateOptions();
    if(!options.length) return false;
    linkCreateState = state;
    createMenu.classList.remove('open');
    linkCreateMenu.classList.add('reference-create-menu');
    linkCreateMenu.innerHTML = `<div class="menu-section-title">引用该节点生成</div>${options.map(opt => `<button class="menu-btn ${opt.type === 'refText' ? 'featured' : ''}" data-link-create="${escapeAttr(opt.type)}"><i data-lucide="${escapeAttr(opt.icon)}" class="w-4 h-4"></i><span><strong>${escapeHtml(opt.label)}</strong>${opt.desc ? `<small>${escapeHtml(opt.desc)}</small>` : ''}</span></button>`).join('')}`;
...
function openGeneratorNodeMenu(nodeId, clientX, clientY){
    ...
    const outputOptions = canvasReferenceCreateOptions();
    const buttonsHtml = (options, kind) => `${options.map(opt => `<button class="menu-btn ${opt.type === 'refText' ? 'featured' : ''}" data-link-create="${escapeAttr(opt.type)}" data-link-kind="${kind}" title="${escapeAttr(opt.label)}"><i data-lucide="${escapeAttr(opt.icon)}"></i><span><strong>${escapeHtml(opt.label)}</strong>${opt.desc ? `<small>${escapeHtml(opt.desc)}</small>` : ''}</span></button>`).join('')}`;
    ...
    nodeOutputMenu.classList.add('reference-create-menu');
    nodeOutputMenu.innerHTML = `<div class="menu-section-title">引用该节点生成</div>${buttonsHtml(outputOptions, 'out')}`;
    const menuWidth = 300;
    const outputLeft = Math.min(window.innerWidth - menuWidth - 12, (rect?.right || clientX) + 18);
    const menuTop = Math.max(12, Math.min(window.innerHeight - 440, (rect?.top || clientY) + 24));
    nodeOutputMenu.style.left = `${outputLeft}px`;
    nodeOutputMenu.style.top = `${menuTop}px`;
    nodeOutputMenu.classList.add('open');
    [nodeOutputMenu].forEach(menu => menu.querySelectorAll('[data-link-create]').forEach(btn => {
        btn.onclick = e => {
            e.stopPropagation();
...
function closeLinkCreateMenu(){
    linkCreateMenu.classList.remove('open');
    linkCreateMenu.classList.remove('reference-create-menu');
    ...
    nodeInputMenu.classList.remove('reference-create-menu');
    nodeOutputMenu.classList.remove('reference-create-menu');
    nodeInputMenu.innerHTML = '';
...
function createNodeByType(type, point){
    if(type === 'refText') return addPromptNode(point);
    if(type === 'refImage') return addComfyNode(point, {
        mode:'text',
        creativeType:'text-to-image',
        title:'图片生成',
        w:980,
        h:340
    });
    if(type === 'refImageEdit') return addComfyNode(point, {
        mode:'edit',
        creativeType:'edit',
        title:'图片编辑器',
        w:980,
    ...
    if(type === 'refVideo') return addVideoNode(point);
...
function menuAdd(type){
    closeCreateMenu();
    if(type === 'creativeText') return addCanvasCreativeNode('text');
    if(type === 'creativeImage') return addCanvasCreativeNode('edit');
    if(type === 'creativeVideo') return addVideoNode(menuPoint);
    if(type === 'image') return addImageNode(menuPoint);
    if(type === 'prompt') return addPromptNode(menuPoint);
    if(type === 'upload') return pickCanvasUploadAtPoint(menuPoint);
    if(type === 'uploadVideo') return pickCanvasUploadAtPoint(menuPoint, 'video/*', true);
    if(type === 'uploadAudio') return pickCanvasUploadAtPoint(menuPoint, 'audio/*', true);
}
...
async function uploadMediaFiles(files, point, onlyImages=false, opts={}){
    ...
    const data = await uploadAiReferenceForm(form, langIsEn() ? 'Media upload failed' : '素材上传失败');
...
async function applyImageGridSplit(){
    ...
    if(files.length){
        ...
        const cardH = 220;
        const gap = 22;
        const cols = Math.min(4, Math.max(1, Math.ceil(Math.sqrt(files.length))));
        const p = imageEditorOutputPoint(node, 0);
        selected.clear();
        files.forEach((file, i) => {
            if(!file?.url) return;
            const next = {
                id:uid('img'),
                type:'image',
                x:p.x + (i % cols) * (cardW + gap),
                y:p.y + Math.floor(i / cols) * (cardH + gap),
                w:cardW,
                h:cardH,
                url:file.url,
...
function mountNodeElement(node){
    if(!node || node.type === 'output') return null;
    try {
        const fresh = renderNode(node);
        const current = nodesEl.querySelector(`.node[data-id="${CSS.escape(node.id)}"]`);
        if(current) current.replaceWith(fresh);
        else nodesEl.appendChild(fresh);
        return fresh;
    } catch(err){
        console.error('[canvas] mountNodeElement failed:', node?.id, node?.type, err);
        return null;
    }
}
function removeNodeElements(ids=[]){
    (ids || []).forEach(id => {
...
function pendingPreviewSizeFromRefs(refs=[]){
    for(const ref of refs || []){
        ...
        const url = ref?.url;
        if(!url) continue;
        const node = nodes.find(n => n.type === 'image' && n.url === url);
...
function renderNode(node){
    ...
    if(node?.type === 'output') throw new Error('Deprecated output node');
    ...
    const composerStateClass = isCanvasGeneratorSurface(node) ? (isCanvasGeneratorComposerOpen(node) ? 'composer-open' : 'composer-closed') : '';
    el.className = `node ${node.type}-node ${node.url ? 'has-image' : ''} ${hasFixedSize ? 'sized' : ''} ${composerStateClass} ${selected.has(node.id) ? 'selected' : ''}`;
    ...
    const canInput = ['generator','comfy','ltxDirector','llm','msgen','video','rh'].includes(node.type) || (node.type === 'loop' && (node.imageInput || node.showPrompt));
    ...
    const showResizeHandle = !isCanvasGeneratorSurface(node);
    if(showResizeHandle) el.insertAdjacentHTML('beforeend', `<div class="resize-handle" title="${tr('canvas.resize')}"></div>`);
    ...
    if(resizeHandle) resizeHandle.onmousedown = e => { if(e.button === 0 && !e.shiftKey) startNodeResize(e, node); };
    ...
    if(isCanvasGeneratorSurface(node)){
        const bindFollowPort = port => {
            if(!port) return;
            port.addEventListener('mousemove', e => {
                ...
                const pct = Math.max(10, Math.min(90, (localY / rect.height) * 100));
                const clamp = (value, min, max) => Math.max(min, Math.min(max, value));
                const nextX = port.classList.contains('in')
                    ? clamp(localX - 33, -124, -42)
                    : clamp(rect.width - localX - 33, -124, -42);
                port.style.setProperty('--port-y', `${pct.toFixed(2)}%`);
                port.style.setProperty('--port-x', `${Math.round(nextX)}px`);
                scheduleLinksRender();
            });
    ...
    if(del){
        ...
        del.onclick = e => {
            ...
            if(pid){
                ...
                node.generatedOutputs = (node.generatedOutputs || []).filter(item => outputUrlValue(item) !== url);
...
async function uploadFilesToLibrary(files, libraryId, categoryId){
    ...
    const uploaded = await uploadAiReferenceForm(form, langIsEn() ? 'Asset upload failed' : '素材上传失败');
...
function renderVideoImageInputs(list, node, imageInputs){
    if(!list) return;
    list.innerHTML = imageInputs.length ? '' : canvasEmptyImageStageHtml();
    imageInputs.forEach((src, i) => {
        const item = document.createElement('div');
...
function toggleComfyRandom(nodeId, fieldId){
    ...
    const active = comfyRandomActive(node, fieldId);
    const el = nodesEl?.querySelector(`.node[data-id="${CSS.escape(node.id)}"]`);
    const btn = el?.querySelector(`[data-comfy-random="${CSS.escape(fieldId)}"]`);
    if(btn){
        btn.classList.toggle('active', active);
...
function canvasEmptyImageStageHtml(){
    return `<div class="canvas-gen-empty-image" title="${langIsEn() ? 'Click to edit' : '点击编辑'}"><i data-lucide="image" class="w-16 h-16"></i></div>`;
}
function openCanvasInputPanel(nodeId, event=null){
    event?.preventDefault?.();
    event?.stopPropagation?.();
    const node = nodes.find(n => n.id === nodeId);
    if(!isCanvasGeneratorSurface(node)) return;
    collapseOpenComposerPanels(node.id, false);
    node.composerOpen = true;
    selected.clear();
    selected.add(node.id);
    if(!syncCanvasGeneratorComposerDom(node)) refreshNodes([node.id]);
    refreshSelectionVisuals();
    scheduleSave();
}
function closeCanvasInputPanel(nodeId, event=null){
    event?.preventDefault?.();
    event?.stopPropagation?.();
    const node = nodes.find(n => n.id === nodeId);
    if(!isCanvasGeneratorSurface(node)) return;
    if(canvasComposerPopover.nodeId === node.id) canvasComposerPopover = {nodeId:'', kind:''};
    node.composerOpen = false;
    if(!syncCanvasGeneratorComposerDom(node)) refreshNodes([node.id]);
    scheduleSave();
}
function collapseOpenComposerPanels(exceptNodeId='', persist=true){
    const changed = [];
    const popoverNodeId = canvasComposerPopover.nodeId;
    let popoverClosed = false;
    if(canvasComposerPopover.nodeId && canvasComposerPopover.nodeId !== exceptNodeId) {
        canvasComposerPopover = {nodeId:'', kind:''};
        popoverClosed = true;
    }
    nodes.forEach(node => {
        if(!isCanvasGeneratorSurface(node) || !node.composerOpen || node.id === exceptNodeId) return;
        node.composerOpen = false;
        changed.push(node.id);
    });
    if(!changed.length) {
        if(popoverClosed && popoverNodeId){
            const node = nodes.find(n => n.id === popoverNodeId);
            if(node && !syncCanvasComposerOptionBar(node)) refreshNodes([popoverNodeId]);
            if(persist) scheduleSave();
            return true;
        }
        return false;
    }
    const missing = changed.filter(id => {
        const node = nodes.find(n => n.id === id);
        return !syncCanvasGeneratorComposerDom(node);
    });
    if(missing.length) refreshNodes(missing);
    if(persist) scheduleSave();
    return true;
}
function syncCanvasGeneratorComposerDom(node){
    if(!isCanvasGeneratorSurface(node) || !nodesEl) return false;
    normalizeApiNodeLayout(node);
    const el = nodesEl.querySelector(`.node[data-id="${CSS.escape(node.id)}"]`);
    if(!el) return false;
    const open = isCanvasGeneratorComposerOpen(node);
    const size = defaultNodeSize(node.type);
    el.style.width = `${node.w || size.w}px`;
    if(node.h || size.h) el.style.height = `${node.h || size.h}px`;
    el.classList.toggle('composer-open', open);
    el.classList.toggle('composer-closed', !open);
    const shell = el.querySelector('.canvas-gen-shell');
    shell?.classList.toggle('composer-open', open);
    shell?.classList.toggle('composer-closed', !open);
    const composer = el.querySelector('.canvas-gen-composer');
    if(!composer) return false;
    composer.classList.toggle('is-open', open);
    composer.classList.toggle('is-collapsed', !open);
    scheduleMinimapRender();
    return true;
}
function bindCanvasInputPanelToggle(wrap, node){
    const stage = wrap.querySelector('.canvas-gen-stage');
    let suppressNextStageClick = false;
    stage?.addEventListener('mousedown', event => {
        if(isCanvasGeneratorComposerOpen(node)) return;
        ...
        const move = moveEvent => {
            ...
            if(distance > 3) start.moved = true;
            if(!start.moved) return;
            moveEvent.preventDefault();
            node.x = start.ox + (moveEvent.clientX - start.x) / viewport.scale;
            node.y = start.oy + (moveEvent.clientY - start.y) / viewport.scale;
            const el = nodesEl.querySelector(`.node[data-id="${CSS.escape(node.id)}"]`);
            if(el){
                el.style.left = `${node.x}px`;
                el.style.top = `${node.y}px`;
        ...
        const up = upEvent => {
            ...
            if(start.moved){
                ...
                updateGroupMembership([node]);
                scheduleSave();
            } else {
                openCanvasInputPanel(node.id, upEvent);
            }
        };
    ...
    if(closeBtn) closeBtn.onclick = event => closeCanvasInputPanel(node.id, event);
}
function canvasGenerationModeMeta(mode){
    const map = {
        text:{title:'文生图', subtitle:'描述任何你想生成的内容', icon:'sparkles', stage:'Text'},
        edit:{title:'图生图', subtitle:'上传或连接参考图后编辑', icon:'image-up', stage:'Image'},
        enhance:{title:'细节增强', subtitle:'选择图片后进行非破坏式增强', icon:'zap', stage:'Image'},
        angle:{title:'角度控制', subtitle:'基于参考图生成新视角', icon:'box', stage:'Image'},
        custom:{title:'自定义', subtitle:'使用当前工作流生成', icon:'workflow', stage:'Media'}
    };
...
function renderComfyBody(node){
    ...
    const needsMedia = mode !== 'text' && !(mode === 'custom' && !mediaFieldCount);
    const modeMeta = canvasGenerationModeMeta(mode);
    const hasInlineOutput = hasInlineGeneratedContent(node);
    const composerOpen = isCanvasGeneratorComposerOpen(node);
    const outputHasVideo = inlineGeneratedOutputItems(node).some(item => mediaKindForOutputItem(item) === 'video');
    const stageIcon = hasInlineOutput && outputHasVideo ? 'play-square' : 'image';
    ...
    wrap.innerHTML = `
        <div class="canvas-gen-shell ${composerOpen ? 'composer-open' : 'composer-closed'}">
            ...
            <div class="canvas-gen-composer ${composerOpen ? 'is-open' : 'is-collapsed'}">
                <div class="canvas-gen-composer-tools">
                    <button type="button" class="canvas-gen-icon active" title="${modeMeta.title}"><i data-lucide="${modeMeta.icon}" class="w-4 h-4"></i></button>
                    <span class="canvas-gen-tool-divider"></span>
                    ...
                    <span class="canvas-gen-mode-copy">${modeMeta.subtitle}</span>
                    <button type="button" class="canvas-gen-panel-close" data-close-composer title="${langIsEn() ? 'Collapse' : '收起'}"><i data-lucide="x" class="w-4 h-4"></i></button>
                </div>
                <div class="prompt-list"></div>
                <div class="gen-settings comfy-settings"></div>
                <div class="canvas-gen-bottom">
                    ${canvasComposerRunButtonHtml(node, 'comfy-run')}
                </div>
                ${retryBarHtml(node)}
            </div>
        </div>
    ...
    if(hasInlineOutput){
        ...
        stageContent.innerHTML = '<div class="input-list"></div>';
        if(needsMedia) renderComfyImages(stageContent.querySelector('.input-list'), node, mode === 'custom' ? mediaInputs : imageInputs);
        else stageContent.querySelector('.input-list').innerHTML = canvasEmptyImageStageHtml();
    }
...
async function runRhNode(nodeId, opts={}){
    ...
    outputTarget._pending = [...(outputTarget._pending || []), makePendingForRun(pendingId, run, node, {refs:media.refs, cascadeTargetId})];
    ...
    try {
        ...
        const meta = collectRunMeta(outputTarget, pendingId);
        outputTarget._pending = (outputTarget._pending || []).filter(p => p.id !== pendingId);
        if(out) appendOutputImages(out, outputs, media.refs[0], [meta]);
...
async function runRhModelNode(node, opts={}){
    ...
    if(!model){
        alert(langIsEn() ? 'Please add a RunningHub model API in API settings first.' : '请先在 API 设置里添加 RunningHub 模型 API');
        return;
    }
    node.rhModel = model;
    node.model = model;
    ...
    node._pending = [
        ...(node._pending || []),
        ...pendingIds.map(id => makePendingForRun(id, run, node, {refs, requestSize:payload.size, cascadeTargetId}))
    ];
    if(!opts.cascade) node.running = true;
    refreshRunNodes(node, null);
    try {
        const taskInfos = await Promise.all(pendingIds.map(() => createCanvasImageTask(payload, {cascadeTargetId})));
        const outputs = [];
        ...
        const metas = collectRunMetas(node, pendingIds);
        node._pending = (node._pending || []).filter(p => !pendingIds.includes(p.id));
        mergeGeneratedOutputs(node, outputs, Boolean(opts.cascade));
        addGenerationLog({run, outputs, runMs:Math.max(...metas.map(m => m.runMs || 0), 0)});
        node.runStatus = 'done';
        node.runError = '';
        scheduleSave();
    } catch(err) {
        ...
        node.runStatus = 'failed';
        node.runError = err.message || String(err);
        addGenerationLog({run, outputs:[], runMs:Math.max(...metas.map(m => m.runMs || 0), 0), error:node.runError});
        scheduleSave();
        if(isCascadeAbortError(err)){
            if(opts.cascade) throw err;
            return;
        ...
        if(opts.cascade) throw err;
        showErrorModal(node.runError || tr('canvas.generationFailed'), tr('canvas.apiFailed'));
    } finally {
        node.running = false;
        refreshRunNodes(node, null);
    }
}
function canvasComposerModelOptions(node){
    const mode = node?.mode || 'text';
    const provider = composerApiProvider();
    ...
    if(['text', 'edit', 'enhance', 'angle'].includes(mode)){
        if(apiModels.length){
            return apiModels.map((model, index) => ({
                id:model,
                label:composerModelLabel(model),
                icon:composerModelIcon(model),
                quality:composerModelQuality(model),
                badge:'',
                type:'api-image',
...
function canvasComposerShortLabel(value, fallback=''){
    const text = String(value || fallback || '').replace(/^Tongyi-MAI\//, '').replace(/^black-forest-labs\//, '').trim();
    if(!text) return fallback || '';
    return text.length > 13 ? `${text.slice(0, 10)}...` : text;
}
function canvasComposerActiveModel(node){
    const options = canvasComposerModelOptions(node);
    const selected = String(node?.composerModel || '').trim();
    return options.find(item => item.id === selected && !item.disabled) || options.find(item => !item.disabled) || options[0] || null;
}
function canvasComposerModelWorkflow(node){
    return canvasComposerActiveModel(node)?.workflow || 'Z-Image.json';
}
function canvasComposerModelType(node){
    return canvasComposerActiveModel(node)?.type || 'zimage';
}
function canvasComposerSizePreset(node){
    const selected = String(node?.composerQuality || '').trim();
    if(['512','1024','1536','2048'].includes(selected)) return selected;
    const w = Number(node?.width || 1024);
    const h = Number(node?.height || 1024);
    const maxSide = Math.max(w, h);
    if(maxSide <= 512) return '512';
    if(maxSide <= 1024) return '1024';
    if(maxSide <= 1536) return '1536';
    if(maxSide <= 2048) return '2048';
    return 'custom';
}
function canvasComposerQualityLabel(value){
    const map = {512:'512P', 1024:'1K', 1536:'2K', 2048:'4K', custom:langIsEn() ? 'Custom' : '自定义'};
    return map[String(value)] || map[value] || String(value || '1K');
}
function canvasComposerAspectValue(node){
    return String(node?.composerAspect || 'auto');
}
function canvasComposerAspectLabel(value='auto'){
    if(value === 'auto') return langIsEn() ? 'Auto' : '自适应';
    return value || (langIsEn() ? 'Auto' : '自适应');
}
function canvasComposerSizeLabel(node){
    const quality = canvasComposerSizePreset(node);
    const aspect = canvasComposerAspectLabel(canvasComposerAspectValue(node));
    if(quality !== 'custom') return `${aspect} · ${canvasComposerQualityLabel(quality)}`;
    const w = Number(node?.width || 1024);
    const h = Number(node?.height || 1024);
    return `${w}×${h}`;
}
function canvasComposerApplyQuality(node, quality, aspect=canvasComposerAspectValue(node)){
    ...
    const pair = ratioMap[aspect] || ratioMap.auto;
    const round64 = value => Math.max(256, Math.round(value / 64) * 64);
    let w = size;
    let h = size;
    if(pair[0] > pair[1]){
        w = size;
        h = round64(size * pair[1] / pair[0]);
    } else if(pair[1] > pair[0]){
        h = size;
        w = round64(size * pair[0] / pair[1]);
    }
    node.composerQuality = String(size);
...
function canvasComposerStyleOptions(selected=''){
    const options = [
        ['', langIsEn() ? 'Style' : '风格'],
        ['realistic', langIsEn() ? 'Realistic' : '写实'],
        ['product', langIsEn() ? 'Product' : '产品'],
        ['interior', langIsEn() ? 'Interior' : '室内'],
        ['cinematic', langIsEn() ? 'Cinematic' : '电影感']
    ];
    return options.map(([value, label]) => `<option value="${escapeAttr(value)}" ${String(selected || '') === value ? 'selected' : ''}>${escapeHtml(label)}</option>`).join('');
}
function canvasComposerStyleLabel(value=''){
    const map = {
        realistic:langIsEn() ? 'Realistic' : '写实',
        product:langIsEn() ? 'Product' : '产品',
        interior:langIsEn() ? 'Interior' : '室内',
    ...
    return map[value] || (langIsEn() ? 'Style' : '风格');
}
function canvasComposerStylePrompt(value=''){
    const map = {
        realistic:'realistic lighting, natural materials, detailed textures',
        product:'clean product design rendering, refined materials, studio lighting',
        interior:'high-end interior design visualization, balanced composition, natural light',
        cinematic:'cinematic composition, controlled contrast, filmic color grading'
    };
    return map[value] || '';
}
function canvasComposerCameraOptions(selected=''){
    const options = [
        ['', langIsEn() ? 'Camera' : '镜头'],
        ['sony-venice', 'Sony Venice'],
        ['arri-alexa', 'ARRI Alexa'],
        ['canon-cinema', 'Canon Cinema'],
        ['wide-angle', langIsEn() ? 'Wide Angle' : '广角']
    ];
    return options.map(([value, label]) => `<option value="${escapeAttr(value)}" ${String(selected || '') === value ? 'selected' : ''}>${escapeHtml(label)}</option>`).join('');
}
function canvasComposerCameraLabel(value=''){
    const map = {
        'sony-venice':'Sony Veni...',
        'arri-alexa':'ARRI Alexa',
        'canon-cinema':'Canon Cine...',
        'wide-angle':langIsEn() ? 'Wide Angle' : '广角'
    };
    return map[value] || (langIsEn() ? 'Camera' : '镜头');
}
function canvasComposerCameraPrompt(value=''){
    const map = {
        'sony-venice':'shot on Sony Venice, refined cinematic optics',
        'arri-alexa':'shot on ARRI Alexa, soft highlight rolloff',
        'canon-cinema':'shot on Canon Cinema camera, natural color science',
        'wide-angle':'wide-angle lens, spacious perspective'
    };
    return map[value] || '';
}
function canvasComposerPromptExtras(node){
    return [canvasComposerStylePrompt(node?.stylePreset), canvasComposerCameraPrompt(node?.cameraPreset)].filter(Boolean).join(', ');
}
function canvasComposerModelLabel(node){
    if(node?.type === 'video') return canvasComposerShortLabel(node.model || 'Video');
    return canvasComposerActiveModel(node)?.label || canvasComposerShortLabel(node?.comfyWorkflow || 'ComfyUI');
}
function canvasComposerPopoverIsOpen(node, kind){
    return canvasComposerPopover.nodeId === node?.id && canvasComposerPopover.kind === kind;
}
function closeCanvasComposerPopover(nodeId=''){
    if(!canvasComposerPopover.nodeId) return;
    const id = canvasComposerPopover.nodeId;
    if(nodeId && id !== nodeId) return;
    canvasComposerPopover = {nodeId:'', kind:''};
    if(id) {
        const node = nodes.find(n => n.id === id);
        if(!syncCanvasComposerOptionBar(node)) refreshNodes([id]);
    }
}
function closeCanvasComposerPopoverOnOutsidePointer(event){
    if(!canvasComposerPopover.nodeId) return;
    const target = event?.target;
    if(target?.closest?.('[data-composer-popover], [data-composer-popup]')) return;
    closeCanvasComposerPopover();
}
function canvasComposerModelPopoverHtml(node){
    const active = canvasComposerActiveModel(node);
    return `<div class="canvas-gen-popover canvas-gen-model-popover" data-composer-popover="model" ${canvasComposerPopoverIsOpen(node, 'model') ? '' : 'hidden'}>
        ${canvasComposerModelOptions(node).map(item => `
            <button type="button" class="canvas-gen-model-option ${active?.id === item.id ? 'active' : ''}" data-composer-model="${escapeAttr(item.id)}" ${item.disabled ? 'disabled' : ''} title="${escapeAttr(item.disabledReason || item.label)}">
                <span class="canvas-gen-model-main">
                    <span class="canvas-gen-model-icon"><i data-lucide="${item.icon || 'sparkles'}" class="w-5 h-5"></i></span>
                    <span class="canvas-gen-model-name">${escapeHtml(item.label)}</span>
                </span>
                <span class="canvas-gen-model-meta"><i data-lucide="gem" class="w-3.5 h-3.5"></i>${escapeHtml(item.quality || '')}</span>
                ${active?.id === item.id ? '<i data-lucide="check" class="canvas-gen-model-check w-5 h-5"></i>' : ''}
            </button>
        `).join('')}
    </div>`;
}
function canvasComposerQualityPopoverHtml(node){
    const activeQuality = canvasComposerSizePreset(node);
    const activeAspect = canvasComposerAspectValue(node);
    ...
    const aspectOptions = [
        {id:'auto', label:langIsEn() ? 'Auto' : '自适应', icon:'scan'},
        ...
        {id:'3:2', label:'3:2', shape:'landscape'},
        {id:'2:3', label:'2:3', shape:'portrait'},
        {id:'5:4', label:'5:4', shape:'landscape'},
        {id:'4:5', label:'4:5', shape:'portrait'},
        {id:'21:9', label:'21:9', shape:'wide'}
    ];
    const thinking = String(node.composerThinkTime || 'high');
    return `<div class="canvas-gen-popover canvas-gen-quality-popover" data-composer-popover="quality" ${canvasComposerPopoverIsOpen(node, 'quality') ? '' : 'hidden'}>
        <div class="canvas-gen-popover-title">${langIsEn() ? 'Quality' : '画质'}</div>
        <div class="canvas-gen-quality-seg">
            ${qualityOptions.map(item => `<button type="button" class="${activeQuality === item.id ? 'active' : ''}" data-composer-quality="${item.id}">${escapeHtml(item.label)}</button>`).join('')}
        </div>
        <div class="canvas-gen-popover-title">${langIsEn() ? 'Aspect Ratio' : '比例'}</div>
        <div class="canvas-gen-ratio-grid">
            ${aspectOptions.map(item => `<button type="button" class="${activeAspect === item.id ? 'active' : ''}" data-composer-aspect="${escapeAttr(item.id)}">
                ${item.icon ? `<i data-lucide="${item.icon}" class="w-5 h-5"></i>` : `<span class="ratio-shape ${item.shape || 'square'}"></span>`}
                <span>${escapeHtml(item.label)}</span>
            </button>`).join('')}
        </div>
        <div class="canvas-gen-popover-title">${langIsEn() ? 'Thinking Time' : '思考时间'}</div>
        <div class="canvas-gen-quality-seg two">
            <button type="button" class="${thinking === 'minimal' ? 'active' : ''}" data-composer-thinking="minimal"><i data-lucide="timer" class="w-4 h-4"></i>${langIsEn() ? 'Minimal' : 'Minimal'}</button>
            <button type="button" class="${thinking === 'high' ? 'active' : ''}" data-composer-thinking="high"><i data-lucide="timer" class="w-4 h-4"></i>${langIsEn() ? 'High' : 'high'}</button>
        </div>
        <div class="canvas-gen-popover-title muted">${langIsEn() ? 'Search' : '搜索'}</div>
        <div class="canvas-gen-quality-seg two muted">
            <button type="button" disabled><span class="radio-dot"></span>${langIsEn() ? 'Web' : '联网'}</button>
            <button type="button" disabled><span class="radio-dot"></span>${langIsEn() ? 'Images' : '图片'}</button>
        </div>
    </div>`;
}
function canvasComposerStylePopoverHtml(node){
    const activeStyle = String(node?.stylePreset || '');
    const options = [
        {id:'', label:langIsEn() ? 'Style' : '风格', icon:'image-plus'},
        {id:'realistic', label:langIsEn() ? 'Realistic' : '写实', icon:'aperture'},
        {id:'product', label:langIsEn() ? 'Product' : '产品', icon:'box'},
        {id:'interior', label:langIsEn() ? 'Interior' : '室内', icon:'sofa'},
        {id:'cinematic', label:langIsEn() ? 'Cinematic' : '电影感', icon:'clapperboard'}
    ];
    return `<div class="canvas-gen-popover canvas-gen-style-popover" data-composer-popover="style" ${canvasComposerPopoverIsOpen(node, 'style') ? '' : 'hidden'}>
        ${options.map(item => `<button type="button" class="${activeStyle === item.id ? 'active' : ''}" data-composer-style="${escapeAttr(item.id)}">
            <i data-lucide="${item.icon}" class="w-4 h-4"></i>
            <span>${escapeHtml(item.label)}</span>
        </button>`).join('')}
    </div>`;
}
function canvasComposerCameraPopoverHtml(node){
    const activeCamera = String(node?.cameraPreset || '');
    const options = [
        {id:'', label:langIsEn() ? 'Camera' : '镜头', icon:'circle-dot'},
        {id:'sony-venice', label:'Sony Venice', icon:'circle-dot'},
        {id:'arri-alexa', label:'ARRI Alexa', icon:'circle-dot'},
        {id:'canon-cinema', label:'Canon Cinema', icon:'circle-dot'},
        {id:'wide-angle', label:langIsEn() ? 'Wide Angle' : '广角', icon:'scan'}
    ];
    return `<div class="canvas-gen-popover canvas-gen-camera-popover" data-composer-popover="camera" ${canvasComposerPopoverIsOpen(node, 'camera') ? '' : 'hidden'}>
        ${options.map(item => `<button type="button" class="${activeCamera === item.id ? 'active' : ''}" data-composer-camera="${escapeAttr(item.id)}">
            <i data-lucide="${item.icon}" class="w-4 h-4"></i>
            <span>${escapeHtml(item.label)}</span>
        </button>`).join('')}
    </div>`;
}
function canvasComposerCountPopoverHtml(node){
    const activeCount = Math.max(1, Math.min(4, Number(node?.count || 1)));
    const options = [4, 2, 1];
    return `<div class="canvas-gen-popover canvas-gen-count-popover" data-composer-popover="count" ${canvasComposerPopoverIsOpen(node, 'count') ? '' : 'hidden'}>
        ${options.map(value => `<button type="button" class="${activeCount === value ? 'active' : ''}" data-composer-count="${value}">${value}&times;</button>`).join('')}
    </div>`;
}
function canvasComposerOptionBar(node){
    ...
    return `
        ...
        ${canvasComposerQualityPopoverHtml(node)}
        ${canvasComposerStylePopoverHtml(node)}
        ${canvasComposerCameraPopoverHtml(node)}
        ${canvasComposerCountPopoverHtml(node)}
        <div class="canvas-gen-options-row">
            <button type="button" class="canvas-gen-token canvas-gen-token-model ${canvasComposerPopoverIsOpen(node, 'model') ? 'active' : ''}" data-composer-popup="model" title="${escapeAttr(modelLabel)}">
                <i data-lucide="${modelIcon}" class="w-4 h-4"></i>
                <span class="canvas-gen-token-label">${escapeHtml(modelLabel)}</span>
            </button>
            <span class="canvas-gen-option-divider"></span>
            <button type="button" class="canvas-gen-token canvas-gen-token-quality ${canUseSize ? '' : 'is-static'} ${canvasComposerPopoverIsOpen(node, 'quality') ? 'active' : ''}" ${canUseSize ? 'data-composer-popup="quality"' : ''} title="${escapeAttr(sizeLabel)}">
                <i data-lucide="scan" class="w-4 h-4"></i>
                <span class="canvas-gen-token-label">${escapeHtml(sizeLabel)}</span>
            </button>
            <span class="canvas-gen-option-divider"></span>
            <button type="button" class="canvas-gen-token canvas-gen-token-style ${canvasComposerPopoverIsOpen(node, 'style') ? 'active' : ''}" data-composer-popup="style" title="${escapeAttr(styleLabel)}">
                <i data-lucide="image-plus" class="w-4 h-4"></i>
                <span class="canvas-gen-token-label">${escapeHtml(styleLabel)}</span>
            </button>
            <span class="canvas-gen-option-divider"></span>
            <button type="button" class="canvas-gen-token canvas-gen-token-camera ${canvasComposerPopoverIsOpen(node, 'camera') ? 'active' : ''}" data-composer-popup="camera" title="${escapeAttr(cameraLabel)}">
                <span class="canvas-gen-lens-dot"></span>
                <span class="canvas-gen-token-label">${escapeHtml(cameraLabel)}</span>
            </button>
            <button type="button" class="canvas-gen-token canvas-gen-mic" disabled title="${langIsEn() ? 'Voice input is not connected yet' : '语音输入暂未接入'}"><i data-lucide="mic" class="w-4 h-4"></i></button>
            <span class="canvas-gen-option-divider"></span>
...
function syncCanvasComposerOptionBar(node){
    ...
    const el = nodesEl.querySelector(`.node[data-id="${CSS.escape(node.id)}"]`);
    const container = el?.querySelector('.comfy-settings, .canvas-video-settings');
    if(!container) return false;
    if(!container.querySelector(':scope > .canvas-gen-options-row')){
        container.insertAdjacentHTML('beforeend', canvasComposerOptionBar(node));
        bindCanvasComposerOptionBar(container, node);
        refreshIcons();
        return true;
    }
    const modelPop = container.querySelector(':scope > [data-composer-popover="model"]');
    const qualityPop = container.querySelector(':scope > [data-composer-popover="quality"]');
    const stylePop = container.querySelector(':scope > [data-composer-popover="style"]');
    const cameraPop = container.querySelector(':scope > [data-composer-popover="camera"]');
    const countPop = container.querySelector(':scope > [data-composer-popover="count"]');
    const fresh = document.createElement('div');
    fresh.innerHTML = canvasComposerOptionBar(node);
    const freshModelPop = fresh.querySelector('[data-composer-popover="model"]');
    const freshQualityPop = fresh.querySelector('[data-composer-popover="quality"]');
    const freshStylePop = fresh.querySelector('[data-composer-popover="style"]');
    const freshCameraPop = fresh.querySelector('[data-composer-popover="camera"]');
    const freshCountPop = fresh.querySelector('[data-composer-popover="count"]');
    if(modelPop && freshModelPop){
        modelPop.hidden = freshModelPop.hidden;
        if(!modelPop.children.length) modelPop.innerHTML = freshModelPop.innerHTML;
    } else if(freshModelPop) {
        container.insertBefore(freshModelPop, container.querySelector(':scope > .canvas-gen-options-row'));
    }
    if(qualityPop && freshQualityPop){
        qualityPop.hidden = freshQualityPop.hidden;
        if(!qualityPop.children.length) qualityPop.innerHTML = freshQualityPop.innerHTML;
    } else if(freshQualityPop) {
        container.insertBefore(freshQualityPop, container.querySelector(':scope > .canvas-gen-options-row'));
    }
    if(stylePop && freshStylePop){
        stylePop.hidden = freshStylePop.hidden;
        if(!stylePop.children.length) stylePop.innerHTML = freshStylePop.innerHTML;
    } else if(freshStylePop) {
        container.insertBefore(freshStylePop, container.querySelector(':scope > .canvas-gen-options-row'));
    }
    if(cameraPop && freshCameraPop){
        cameraPop.hidden = freshCameraPop.hidden;
        if(!cameraPop.children.length) cameraPop.innerHTML = freshCameraPop.innerHTML;
    } else if(freshCameraPop) {
        container.insertBefore(freshCameraPop, container.querySelector(':scope > .canvas-gen-options-row'));
    }
    if(countPop && freshCountPop){
        countPop.hidden = freshCountPop.hidden;
        if(!countPop.children.length) countPop.innerHTML = freshCountPop.innerHTML;
    ...
    if(modelBtn){
        modelBtn.classList.toggle('active', canvasComposerPopoverIsOpen(node, 'model'));
        modelBtn.title = modelLabel;
        const icon = modelBtn.querySelector('i[data-lucide]');
        if(icon && model?.icon) icon.setAttribute('data-lucide', model.icon);
        const label = modelBtn.querySelector('.canvas-gen-token-label');
    ...
    const qualityBtn = row?.querySelector('[data-composer-popup="quality"], .canvas-gen-token-quality');
    if(qualityBtn){
        qualityBtn.classList.toggle('active', canvasComposerPopoverIsOpen(node, 'quality'));
        qualityBtn.title = sizeLabel;
        const label = qualityBtn.querySelector('.canvas-gen-token-label');
        if(label) label.textContent = sizeLabel;
    }
    const styleToken = row?.querySelector('[data-composer-popup="style"]');
    const styleLabel = canvasComposerStyleLabel(node?.stylePreset || '');
    if(styleToken){
        styleToken.classList.toggle('active', canvasComposerPopoverIsOpen(node, 'style'));
        styleToken.title = styleLabel;
        const label = styleToken.querySelector('.canvas-gen-token-label');
        if(label) label.textContent = styleLabel;
    }
    const cameraToken = row?.querySelector('[data-composer-popup="camera"]');
    const cameraLabel = canvasComposerCameraLabel(node?.cameraPreset || '');
    if(cameraToken){
        cameraToken.classList.toggle('active', canvasComposerPopoverIsOpen(node, 'camera'));
        cameraToken.title = cameraLabel;
        const label = cameraToken.querySelector('.canvas-gen-token-label');
        if(label) label.textContent = cameraLabel;
    }
    const count = Math.max(1, Math.min(8, Number(node?.count || 1)));
    const countBtn = row?.querySelector('[data-count-cycle]');
    if(countBtn) {
...
function syncCanvasComposerPopoverState(container, node){
    const activeModel = canvasComposerActiveModel(node);
    container.querySelectorAll('[data-composer-model]').forEach(btn => {
        const active = btn.dataset.composerModel === activeModel?.id;
        btn.classList.toggle('active', active);
        btn.querySelector('.canvas-gen-model-check')?.remove();
        if(active && !btn.querySelector('.canvas-gen-model-check')){
            btn.insertAdjacentHTML('beforeend', '<i data-lucide="check" class="canvas-gen-model-check w-5 h-5"></i>');
        }
    });
    const activeQuality = canvasComposerSizePreset(node);
    container.querySelectorAll('[data-composer-quality]').forEach(btn => {
        btn.classList.toggle('active', btn.dataset.composerQuality === activeQuality);
    });
    const activeAspect = canvasComposerAspectValue(node);
    container.querySelectorAll('[data-composer-aspect]').forEach(btn => {
        btn.classList.toggle('active', btn.dataset.composerAspect === activeAspect);
    });
    const activeThinking = String(node.composerThinkTime || 'high');
    container.querySelectorAll('[data-composer-thinking]').forEach(btn => {
        btn.classList.toggle('active', btn.dataset.composerThinking === activeThinking);
    });
    const activeStyle = String(node?.stylePreset || '');
    container.querySelectorAll('[data-composer-style]').forEach(btn => {
        btn.classList.toggle('active', btn.dataset.composerStyle === activeStyle);
    });
    const activeCamera = String(node?.cameraPreset || '');
    container.querySelectorAll('[data-composer-camera]').forEach(btn => {
        btn.classList.toggle('active', btn.dataset.composerCamera === activeCamera);
    });
    const activeCount = String(Math.max(1, Math.min(4, Number(node?.count || 1))));
    container.querySelectorAll('[data-composer-count]').forEach(btn => {
        btn.classList.toggle('active', btn.dataset.composerCount === activeCount);
    });
}
function positionCanvasComposerPopoverOverToken(container, kind, tokenSelector){
    const pop = container?.querySelector?.(`[data-composer-popover="${kind}"]`);
    const row = container?.querySelector?.(':scope > .canvas-gen-options-row');
    const btn = row?.querySelector?.(tokenSelector);
    if(!pop || !row || !btn) return;
    const popWidth = pop.offsetWidth || (kind === 'count' ? 70 : 220);
    const left = row.offsetLeft + btn.offsetLeft + ((btn.offsetWidth || 58) - popWidth) / 2;
    const bottom = container.clientHeight - (row.offsetTop + btn.offsetTop) + 8;
    pop.style.left = `${Math.max(0, Math.round(left))}px`;
    pop.style.right = 'auto';
    pop.style.bottom = `${Math.max(54, Math.round(bottom))}px`;
}
function positionCanvasComposerCountPopover(container){
    positionCanvasComposerPopoverOverToken(container, 'count', '[data-count-cycle]');
}
function canvasComposerRunButtonHtml(node, className='comfy-run'){
    const count = Math.max(1, Math.min(8, Number(node?.count || 1)));
    const cost = Math.max(1, count) * 20;
    const icon = node?.running ? 'loader-2' : 'arrow-up';
    return `<button class="${className} canvas-gen-run ${node?.running ? 'running' : ''}" ${node?.running ? 'disabled' : ''} title="${node?.running ? tr('canvas.comfyRunning') : tr('canvas.comfyRun')}"><span class="canvas-gen-cost"><i data-lucide="atom" class="w-4 h-4"></i>${cost}</span><span class="canvas-gen-arrow"><i data-lucide="${icon}" class="w-4 h-4"></i></span></button>`;
}
function toggleCanvasComposerOption(nodeId, kind, event=null){
    event?.preventDefault?.();
    event?.stopPropagation?.();
    const node = nodes.find(n => n.id === nodeId);
    if(!node) return;
    canvasComposerPopover = canvasComposerPopoverIsOpen(node, kind) ? {nodeId:'', kind:''} : {nodeId:node.id, kind};
    if(!syncCanvasComposerOptionBar(node)) refreshNodes([node.id]);
}
function selectCanvasComposerModel(nodeId, modelId, event=null){
    event?.preventDefault?.();
    event?.stopPropagation?.();
    const node = nodes.find(n => n.id === nodeId);
    if(!node) return;
    const option = canvasComposerModelOptions(node).find(item => item.id === modelId);
    if(!option || option.disabled) return;
    node.composerModel = option.id;
    canvasComposerPopover = {nodeId:'', kind:''};
    scheduleSave();
    if(!syncCanvasComposerOptionBar(node)) refreshNodes([node.id]);
}
function setCanvasComposerQuality(nodeId, quality, event=null){
    event?.preventDefault?.();
    event?.stopPropagation?.();
    const node = nodes.find(n => n.id === nodeId);
    if(!node) return;
    canvasComposerApplyQuality(node, quality || '1024');
    scheduleSave();
    if(!syncCanvasComposerOptionBar(node)) refreshNodes([node.id]);
}
function setCanvasComposerAspect(nodeId, aspect, event=null){
    event?.preventDefault?.();
    event?.stopPropagation?.();
    const node = nodes.find(n => n.id === nodeId);
    if(!node) return;
    canvasComposerApplyQuality(node, canvasComposerSizePreset(node), aspect || 'auto');
    scheduleSave();
    if(!syncCanvasComposerOptionBar(node)) refreshNodes([node.id]);
}
function setCanvasComposerThinking(nodeId, thinking, event=null){
    event?.preventDefault?.();
    event?.stopPropagation?.();
    const node = nodes.find(n => n.id === nodeId);
    if(!node) return;
    node.composerThinkTime = thinking || 'high';
    scheduleSave();
    if(!syncCanvasComposerOptionBar(node)) refreshNodes([node.id]);
}
function setCanvasComposerStyle(nodeId, style, event=null){
    event?.preventDefault?.();
    event?.stopPropagation?.();
    const node = nodes.find(n => n.id === nodeId);
    if(!node) return;
    node.stylePreset = style || '';
    canvasComposerPopover = {nodeId:'', kind:''};
    scheduleSave();
    if(!syncCanvasComposerOptionBar(node)) refreshNodes([node.id]);
}
function setCanvasComposerCamera(nodeId, camera, event=null){
    event?.preventDefault?.();
    event?.stopPropagation?.();
    const node = nodes.find(n => n.id === nodeId);
    if(!node) return;
    node.cameraPreset = camera || '';
    canvasComposerPopover = {nodeId:'', kind:''};
    scheduleSave();
    if(!syncCanvasComposerOptionBar(node)) refreshNodes([node.id]);
}
function setCanvasComposerCount(nodeId, count, event=null){
    event?.preventDefault?.();
    event?.stopPropagation?.();
    const node = nodes.find(n => n.id === nodeId);
    if(!node) return;
    node.count = Math.max(1, Math.min(4, Number(count || 1)));
    canvasComposerPopover = {nodeId:'', kind:''};
    scheduleSave();
    if(!syncCanvasComposerOptionBar(node)) refreshNodes([node.id]);
}
window.toggleCanvasComposerOption = toggleCanvasComposerOption;
window.selectCanvasComposerModel = selectCanvasComposerModel;
window.setCanvasComposerQuality = setCanvasComposerQuality;
window.setCanvasComposerAspect = setCanvasComposerAspect;
window.setCanvasComposerThinking = setCanvasComposerThinking;
window.setCanvasComposerStyle = setCanvasComposerStyle;
window.setCanvasComposerCamera = setCanvasComposerCamera;
window.setCanvasComposerCount = setCanvasComposerCount;
function handleCanvasComposerPopoverWheel(event){
    const pop = event.currentTarget?.matches?.('[data-composer-popover]')
        ? event.currentTarget
        : event.target?.closest?.('[data-composer-popover]');
    if(!pop) return;
    event.stopPropagation();
    event.stopImmediatePropagation?.();
    const canScrollY = pop.scrollHeight > pop.clientHeight + 1;
    if(canScrollY){
        if(event.cancelable) event.preventDefault();
        const unit = event.deltaMode === 1 ? 16 : (event.deltaMode === 2 ? pop.clientHeight : 1);
        const maxY = Math.max(0, pop.scrollHeight - pop.clientHeight);
        pop.scrollTop = Math.max(0, Math.min(maxY, pop.scrollTop + event.deltaY * unit));
        if(event.deltaX) pop.scrollLeft += event.deltaX * unit;
        return;
    }
}
...
function bindCanvasComposerOptionBar(container, node){
    container.querySelectorAll('[data-composer-popup]').forEach(btn => {
        btn.onmousedown = e => e.stopPropagation();
        btn.onclick = e => {
            e.preventDefault();
            e.stopPropagation();
            toggleCanvasComposerOption(node.id, btn.dataset.composerPopup || '', e);
        };
    });
    container.querySelectorAll('[data-composer-popover]').forEach(pop => {
        pop.onmousedown = e => e.stopPropagation();
        pop.onclick = e => e.stopPropagation();
        if(!pop.dataset.wheelBound){
            pop.addEventListener('wheel', handleCanvasComposerPopoverWheel, {passive:false});
            pop.dataset.wheelBound = '1';
        }
    });
    container.querySelectorAll('[data-composer-model]').forEach(btn => {
        btn.onmousedown = e => e.stopPropagation();
        btn.onclick = e => {
            ...
            selectCanvasComposerModel(node.id, btn.dataset.composerModel || 'banana-2', e);
        };
    });
    container.querySelectorAll('[data-composer-quality]').forEach(btn => {
        btn.onmousedown = e => e.stopPropagation();
        btn.onclick = e => {
            setCanvasComposerQuality(node.id, btn.dataset.composerQuality || '1024', e);
        };
    });
    container.querySelectorAll('[data-composer-aspect]').forEach(btn => {
        btn.onmousedown = e => e.stopPropagation();
        btn.onclick = e => {
            setCanvasComposerAspect(node.id, btn.dataset.composerAspect || 'auto', e);
        };
    });
    container.querySelectorAll('[data-composer-thinking]').forEach(btn => {
        btn.onmousedown = e => e.stopPropagation();
        btn.onclick = e => {
            setCanvasComposerThinking(node.id, btn.dataset.composerThinking || 'high', e);
        };
    });
    container.querySelectorAll('[data-composer-style]').forEach(btn => {
        btn.onmousedown = e => e.stopPropagation();
        btn.onclick = e => {
            setCanvasComposerStyle(node.id, btn.dataset.composerStyle || '', e);
        };
    });
    container.querySelectorAll('[data-composer-camera]').forEach(btn => {
        btn.onmousedown = e => e.stopPropagation();
        btn.onclick = e => {
            setCanvasComposerCamera(node.id, btn.dataset.composerCamera || '', e);
        };
    });
    container.querySelectorAll('[data-composer-count]').forEach(btn => {
        btn.onmousedown = e => e.stopPropagation();
        btn.onclick = e => {
            setCanvasComposerCount(node.id, btn.dataset.composerCount || '1', e);
        };
    });
}
function renderComfySettings(container, node){
    ...
    if(mode === 'text'){
        container.innerHTML = `
            <div class="canvas-gen-prompt-card">
                <textarea class="setting-input canvas-gen-prompt-input" data-field="prompt" placeholder="${langIsEn() ? 'Describe anything you want to generate' : '描述任何你想生成的内容'}">${escapeHtml(node.prompt || '')}</textarea>
            </div>
    ...
    container.querySelectorAll('[data-toggle-field]').forEach(btn => {
        ...
        btn.onclick = e => {
            ...
            node[field] = !node[field];
            btn.classList.toggle('active', Boolean(node[field]));
            const linkedSelect = field === 'enhanceUpscale'
                ? container.querySelector('[data-field="enhanceUpscaleRes"]')
                : (field === 'editUpscale' ? container.querySelector('[data-field="editUpscaleRes"]') : null);
            if(linkedSelect){
                linkedSelect.disabled = !node[field];
                linkedSelect.classList.toggle('opacity-40', !node[field]);
                linkedSelect.classList.toggle('cursor-not-allowed', !node[field]);
            }
            scheduleSave();
        };
    });
...
function updateComfyField(node, input, event){
    ...
    if(paramId){
        ...
        if(type === 'boolean') input.classList.toggle('active', Boolean(node.comfyParams[paramId]));
        scheduleSave();
        return;
    }
    ...
    if(input.type === 'checkbox') {
        ...
        if(field === 'enhanceUpscale' || field === 'editUpscale'){
            const container = input.closest('.comfy-settings, .canvas-video-settings');
            const linkedSelect = container?.querySelector(`[data-field="${field === 'enhanceUpscale' ? 'enhanceUpscaleRes' : 'editUpscaleRes'}"]`);
            if(linkedSelect){
                linkedSelect.disabled = !node[field];
                linkedSelect.classList.toggle('opacity-40', !node[field]);
                linkedSelect.classList.toggle('cursor-not-allowed', !node[field]);
        ...
        if(['stylePreset','cameraPreset'].includes(field)) syncCanvasComposerOptionBar(node);
    }
    scheduleSave();
}
...
let inlineOutputMigrationChanged = false;
function usesInlineGeneratedOutput(node){
    return CANVAS_GENERATOR_TYPES.includes(node?.type);
}
function inlineGeneratedOutputItems(node){
    return (node?.generatedOutputs || []).filter(item => outputUrlValue(item));
}
function hasInlineGeneratedContent(node){
    return usesInlineGeneratedOutput(node) && (inlineGeneratedOutputItems(node).length || (node?._pending || []).length);
}
function renderInlineGeneratedOutputs(container, node){
    if(!container) return;
    const pseudo = {...node, images:inlineGeneratedOutputItems(node)};
    const pendingHtml = (node._pending || []).map(renderPendingOutput).join('');
    container.innerHTML = renderOutputGrid(pseudo, pendingHtml);
    container.querySelectorAll('.output-img-wrap').forEach(wrap => bindOutputWrap(wrap, node));
    bindCanvasPreviewImageFallbacks(container);
    syncCanvasSelectedImageResolution(container);
}
function migrateInlineGeneratedOutputNodes(){
    if(!nodes?.length) return false;
    const outputNodes = nodes.filter(node => node?.type === 'output');
    if(!outputNodes.length) return false;
    const removeOutputIds = new Set(outputNodes.map(node => node.id));
    let changed = false;
    outputNodes.forEach(out => {
        const upstream = (connections || [])
            .filter(conn => conn.to === out.id)
            .map(conn => nodes.find(node => node.id === conn.from))
            .filter(Boolean);
        const downstream = (connections || [])
            .filter(conn => conn.from === out.id)
            .map(conn => nodes.find(node => node.id === conn.to))
            .filter(Boolean);
        const source = upstream.find(node => CANVAS_MEDIA_OUTPUT_TYPES.includes(node.type)) || upstream[0] || null;
        const outputs = [...(out.images || []), ...(out.generatedOutputs || [])].filter(outputUrlValue);
        if(source && CANVAS_MEDIA_OUTPUT_TYPES.includes(source.type)){
            if(outputs.length) mergeGeneratedOutputs(source, outputs, true);
            if(out._pending?.length) source._pending = [...(source._pending || []), ...out._pending];
            if(out.imageComparisons) source.imageComparisons = {...(source.imageComparisons || {}), ...out.imageComparisons};
            downstream.forEach(next => {
                if(next?.id && canConnect(source.id, next.id) && !connections.some(c => c.from === source.id && c.to === next.id)){
                    connections.push({id:uid('c'), from:source.id, to:next.id});
                }
            });
        } else {
            outputs.forEach((item, index) => {
                ...
                const x = Number(out.x || 0) + (index % 4) * 36;
                const y = Number(out.y || 0) + Math.floor(index / 4) * 36;
                nodes.push({
                    id:uid('img'),
                    type:'image',
                    x,
    ...
    nodes = nodes.filter(node => !removeOutputIds.has(node.id));
    connections = connections.filter(conn =>
        !removeOutputIds.has(conn.from) &&
        !removeOutputIds.has(conn.to)
    );
    selected = new Set([...selected].filter(id => !removeOutputIds.has(id)));
    inlineOutputMigrationChanged = true;
    return true;
}
function consumeInlineOutputMigrationChanged(){
    const changed = inlineOutputMigrationChanged;
    inlineOutputMigrationChanged = false;
    return changed;
}
...
async function runGenerator(genId, opts={}){
    ...
    const cascadeTargetId = cascadeTargetIdFromOptions(opts);
    const sources = orderedSources(gen, generatorSources(gen));
    const prompt = [gen.prompt, ...sources.map(s => s.prompt)].filter(Boolean).join('\n\n');
    const refs = imageRefsOnly(sources.flatMap(s => s.refs || []));
    if(!prompt && !refs.length){ alert(tr('canvas.needPromptOrImage')); return; }
...
async function runGeneratorLegacy(genId, opts={}){
    ...
    const count = Math.max(1, Math.min(8, Number(gen.count || 1)));
    let out = null;
    const outputTarget = out || gen;
    const pendingIds = Array.from({length:count}, () => uid('p'));
    const run = runSnapshot(gen, prompt || 'Edit the reference images.', refs);
    const requestSize = await generatorSizeForRun(gen, refs);
    outputTarget._pending = [...(outputTarget._pending||[]), ...pendingIds.map(id => makePendingForRun(id, run, gen, {refs, requestSize}))];
    if(!opts.cascade){
        gen.running = true;
        refreshRunNodes(gen, out);
        setTimeout(() => { gen.running = false; refreshRunNodes(gen, out); }, 2000);
    ...
    try {
        ...
        run.request = results[0] ? requestMetaFromResult(results[0]) : {};
        outputTarget._pending = (outputTarget._pending||[]).filter(p => !pendingIds.includes(p.id));
        if(out) appendOutputImages(out, images, refs[0], metas);
...
async function runVideoNode(nodeId, opts={}){
    ...
    const cascadeTargetId = cascadeTargetIdFromOptions(opts);
    const sources = orderedSources(node, generatorSources(node));
    const prompt = [node.prompt, ...sources.map(s => s.prompt)].filter(Boolean).join('\n\n');
    ...
    const inlineTarget = usesInlineGeneratedOutput(node) ? node : null;
    let out = null;
    const outputTarget = inlineTarget || out;
    const pendingId = uid('p');
    const run = runSnapshot(node, prompt, refs);
    if(outputTarget) outputTarget._pending = [...(outputTarget._pending || []), makePendingForRun(pendingId, run, node, {refs, cascadeTargetId})];
    ...
    try {
        ...
        const meta = collectRunMeta(outputTarget, pendingId);
        if(outputTarget) outputTarget._pending = (outputTarget._pending || []).filter(p => p.id !== pendingId);
        ...
        run.request = requestMetaFromResult(result);
        if(out) appendOutputImages(out, outputUrls, refs[0], [{...meta, kind:'video'}]);
        mergeGeneratedOutputs(node, outputUrls, Boolean(opts.cascade));
        addGenerationLog({run, outputs:outputUrls, runMs:meta.runMs || 0});
        node.runStatus = 'done'; node.runError = '';
        if(out) refreshRunNodes(node, out);
        else render();
        scheduleSave();
    } catch(err) {
        const meta = collectRunMeta(outputTarget, pendingId);
...
async function runLTXDirectorNode(nodeId, opts={}){
    ...
    outputTarget._pending = [...(outputTarget._pending || []), makePendingForRun(pendingId, run, node, {refs, cascadeTargetId})];
    if(!opts.cascade){
        node.running = true;
        refreshRunNodes(node, out);
    ...
    try {
        ...
        const meta = collectRunMeta(outputTarget, pendingId);
        outputTarget._pending = (outputTarget._pending || []).filter(p => p.id !== pendingId);
        if(out) appendOutputImages(out, outputs, refs[0], [meta]);
        mergeGeneratedOutputs(node, outputs, Boolean(opts.cascade));
        addGenerationLog({run, outputs, runMs:meta.runMs || 0});
        node.runStatus = 'done';
        node.runError = '';
...
function composerApiPrompt(node, prompt){
    const mode = node?.mode || 'text';
    const text = String(prompt || '').trim();
    if(text) return text;
    if(mode === 'enhance') return langIsEn() ? 'Enhance image details while preserving the original composition.' : '增强图片细节，保持原始构图和内容。';
    if(mode === 'angle') return langIsEn() ? 'Generate a new view angle based on the reference image.' : '根据参考图生成新的视角。';
    if(mode === 'edit') return langIsEn() ? 'Edit the reference image according to the user intent.' : '根据用户意图编辑参考图片。';
    return text;
}
async function runComposerApiImageNode(node, context={}){
    if(!node || (node.running && !context?.opts?.cascade)) return;
    const option = context.option || canvasComposerActiveModel(node);
    const providerId = resolveImageProviderId(option?.apiProvider || node.apiProvider || composerApiProvider()?.id || '');
    const model = option?.apiModel || option?.id || providerImageModels(providerId)[0] || 'gpt-image-2';
    const refs = imageRefsOnly(context.refs || []);
    const mode = node.mode || 'text';
    const prompt = composerApiPrompt(node, context.prompt || '');
    if(!prompt && !refs.length){ alert(tr('canvas.needPromptOrImage')); return; }
    ...
    node._pending = [
        ...(node._pending || []),
        ...pendingIds.map(id => makePendingForRun(id, run, node, {refs, requestSize, cascadeTargetId}))
    ];
    if(!context?.opts?.cascade){
        node.running = true;
        refreshRunNodes(node, null);
        setTimeout(() => { node.running = false; refreshRunNodes(node, null); }, 2000);
    } else {
        refreshRunNodes(node, null);
    }
    const payload = {
        ...
        model,
        size:requestSize,
        aspect_ratio:aspect === 'auto' ? '' : aspect,
        resolution,
        reference_images:refs.slice(0, CANVAS_REFERENCE_IMAGE_MAX)
    };
    const quality = normalizedImageQuality(node.quality);
    ...
    try {
        ...
        const metas = collectRunMetas(node, pendingIds);
        node._pending = (node._pending || []).filter(p => !pendingIds.includes(p.id));
        mergeGeneratedOutputs(node, outputs, Boolean(context?.opts?.cascade));
        addGenerationLog({run, outputs, runMs:Math.max(...metas.map(m => m.runMs || 0), 0)});
        node.runStatus = 'done';
        node.runError = '';
        node.running = false;
        ...
        refreshRunNodes(node, null);
        scheduleSave();
        if(context?.opts?.cascade) throw err;
        showErrorModal(node.runError || tr('canvas.generationFailed'), tr('canvas.apiFailed'));
    }
}
async function runComfyNode(nodeId, opts={}){
    ...
    const activeComposerModel = canvasComposerActiveModel(node);
    if(activeComposerModel?.type === 'api-image'){
        return runComposerApiImageNode(node, {prompt, refs, option:activeComposerModel, cascadeTargetId, opts});
    }
    const inlineTarget = usesInlineGeneratedOutput(node) ? node : null;
    let out = null;
    const outputTarget = inlineTarget || out;
    ...
    if(outputTarget) outputTarget._pending = [...(outputTarget._pending||[]), makePendingForRun(pendingId, run, node, {refs, requestSize, cascadeTargetId})];
    if(!opts.cascade){
        node.running = true;
        refreshRunNodes(node, out);
        setTimeout(() => { node.running = false; refreshRunNodes(node, out); }, 2000);
    ...
    try {
        ...
        if(mode === 'text'){
            ...
            for(let i = 0; i < count; i++){
                const result = await runQueuedComfyGenerate({
                    ...
                    workflow_json:canvasComposerModelWorkflow(node),
                    type:canvasComposerModelType(node),
                    client_id:CLIENT_ID
                }, {cascadeTargetId});
                if(i === 0) run.request = requestMetaFromResult(result);
                images.push(...comfyResultOutputs(result));
            }
            if(!images.length) throw new Error(noReturnedImage('canvas.comfyText'));
        } else if(mode === 'enhance'){
            ...
            run.taskLabel = '角度控制';
            const inputName = await comfyNameForRef(refs[0]);
            const anglePrompt = prompt || anglePresetPrompt(node.anglePreset);
            const result = await runQueuedComfyGenerate({
                prompt:anglePrompt,
                workflow_json:'2511.json',
                params:{
                    "31": { image:inputName },
                    "11": { prompt:anglePrompt }
                },
                type:'angle',
                client_id:CLIENT_ID
            }, {cascadeTargetId});
            run.request = requestMetaFromResult(result);
            if(result.error) throw new Error(actionFailed('canvas.comfyGenerate', result.error));
            if(!result.images?.length) throw new Error(noReturnedImage('canvas.comfyGenerate'));
        ...
        const meta = collectRunMeta(outputTarget, pendingId);
        if(outputTarget) outputTarget._pending = (outputTarget._pending||[]).filter(p => p.id !== pendingId);
        if(out) appendOutputImages(out, images, refs[0], [meta]);
        ...
        refreshRunNodes(node, out);
        if(opts.cascade) throw err;
        showErrorModal(err.message || actionFailed('canvas.comfyGenerate'), tr('canvas.apiFailed'));
    }
}
...
function findOutputByPendingId(pendingId){
    return nodes.find(n => (n._pending || []).some(p => p.id === pendingId));
}
...
function updateSelectionBox(x, y){
    ...
    if(!selectDrag.moved && Math.hypot(x - selectDrag.sx, y - selectDrag.sy) <= 4) return;
    if(!selectDrag.moved){
...
function finishSelection(){
    if(!selectDrag) return;
    const wasDragSelection = Boolean(selectDrag.moved);
...
function canConnect(fromId, toId){
    ...
    if(CANVAS_GENERATOR_TYPES.includes(from.type)){
        if(from.creativeType && to.type === 'image') return true;
    ...
    if(to.type === 'loop'){
        const allowImage = Boolean(to.imageInput) && ['image','group'].includes(from.type);
        const allowPrompt = Boolean(to.showPrompt) && ['prompt','promptGroup','loop','llm'].includes(from.type);
        return allowImage || allowPrompt;
    }
    if(to.type === 'llm') return ['prompt','loop','promptGroup','llm','image','group'].includes(from.type);
    if(from.type === 'llm') return CANVAS_GENERATOR_TYPES.includes(to.type);
    return CANVAS_GENERATOR_TYPES.includes(to.type) && ['image','prompt','loop','group','promptGroup','llm'].includes(from.type);
}
