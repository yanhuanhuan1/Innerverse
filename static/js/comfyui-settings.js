...
function defaultMiniTestNodes(){
    return [
        ...
        { id:'result_1', type:'result', x:670, y:190 }
    ];
}
...
function renderMiniCanvasPreview(target = previewCard, large = false){
    ...
    const result = miniTestNodes.find(n => n.type === 'result') || defaultMiniTestNodes().find(n => n.type === 'result');
    const resultHtml = runResult
    ...
    target.innerHTML = `
        <div id="miniCanvas" class="mini-canvas ${large ? 'large' : ''}">
            ...
            <div id="miniWorld" class="mini-world" style="transform:translate(${miniView.x}px,${miniView.y}px) scale(${miniView.k})">
                ...
                <div class="mini-card" data-node="${result.id}" style="left:${result.x}px;top:${result.y}px">
                    <span class="mini-port in"></span>
                    <div class="mini-card-head"><i data-lucide="image" class="w-3.5 h-3.5"></i><span>${tr('comfy.resultHere')}</span></div>
