...
function scrollComposerPopoverWithWheel(event){
    ...
    scroller.scrollTop = Math.max(0, Math.min(maxY, scroller.scrollTop + delta));
    return true;
}
composer?.addEventListener('wheel', scrollComposerPopoverWithWheel, {capture:true, passive:false});
document.addEventListener('wheel', event => {
    if(event.target?.closest?.('.composer .model-list, .composer .direct-picker-col, .composer .rh-config-list, .composer .rh-param-list')){
        scrollComposerPopoverWithWheel(event);
    }
